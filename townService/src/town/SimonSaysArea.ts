...
export default class SimonSaysArea extends InteractableArea {
    ...
    public static fromMapObject(mapObject: ITiledMapObject, townEmitter: TownEmitter): SimonSaysArea {
      ...
      return new SimonSaysArea({ pattern: undefined, id: name, round: 0, leaderboard: []}, rect, townEmitter);
    }
    ...
    */
    private async loadLeaderboard(): Promise<void>{
      this._leaderboard = JSON.parse(await fs.readFile('../../../shared/SSleaderboard.json', 'utf-8')) as LeaderboardEntry[];
    }
    /**
    ...
    */
    private leaderboardUpdate(currPlayer: Player, currStreak: number): void{
      ...
      }else{ // this is the updated entry if the player doesn't have a previous entry in the leaderboard
          var updatedEntry: LeaderboardEntry = {
          playerID: currPlayer.id,
          stats: {
            currentSreak: currStreak,
